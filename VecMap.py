...
def main():
    ...
    parser.add_argument('--src_input', default=r'./data/fasttext_embeddings/wiki.multi.en.vec.txt', help='the input source embeddings')
    parser.add_argument('--trg_input', default=r'./data/fasttext_embeddings/wiki.multi.de.vec.txt', help='the input target embeddings')
    parser.add_argument('--src_output', default=r'./data/embedding_new/en.mapped_de_vecmap.txt', help='the output source embeddings')
    parser.add_argument('--trg_output', default=r'./data/embedding_new/de.mapped_en_vecmap.txt', help='the output target embeddings')
    parser.add_argument('--encoding', default='utf-8', help='the character encoding for input/output (defaults to utf-8)')
    parser.add_argument('--precision', choices=['fp16', 'fp32', 'fp64'], default='fp32', help='the floating-point precision (defaults to fp32)')
    parser.add_argument('--cuda', action='store_true', default='cuda', help='use cuda (requires cupy)')
    parser.add_argument('--batch_size', default=8000, type=int, help='batch size (defaults to 10000); does not affect results, larger is usually faster but uses more memory')
    ...
    recommended_type.add_argument('--supervised', metavar='DICTIONARY', default=None, help='recommended if you have a large training dictionary')
    recommended_type.add_argument('--semi_supervised', metavar='DICTIONARY', default=None, help='recommended if you have a small seed dictionary')
    recommended_type.add_argument('--identical', action='store_true', default=False, help='recommended if you have no seed dictionary but can rely on identical words')
    recommended_type.add_argument('--unsupervised', action='store_true', default=True, help='recommended if you have no seed dictionary and do not want to rely on identical words')
    recommended_type.add_argument('--acl2018', action='store_true', default=False, help='reproduce our ACL 2018 system')
    recommended_type.add_argument('--aaai2018', metavar='DICTIONARY', default=False, help='reproduce our AAAI 2018 system')
    recommended_type.add_argument('--acl2017', action='store_true', default=False, help='reproduce our ACL 2017 system with numeral initialization')
    recommended_type.add_argument('--acl2017_seed', metavar='DICTIONARY', default=False, help='reproduce our ACL 2017 system with a seed dictionary')
    recommended_type.add_argument('--emnlp2016', metavar='DICTIONARY', default=False, help='reproduce our EMNLP 2016 system')
    ...
    init_type.add_argument('-d', '--init_dictionary', default=None, metavar='DICTIONARY', help='the training dictionary file (defaults to stdin)')
    init_type.add_argument('--init_identical', action='store_true', default=False, help='use identical words as the seed dictionary')
    init_type.add_argument('--init_numerals', action='store_true', default=False, help='use latin numerals (i.e. words matching [0-9]+) as the seed dictionary')
    init_type.add_argument('--init_unsupervised', action='store_true', default=True, help='use unsupervised initialization')
    ...
    #自学习参数设置（即dictionary induction优化方案）
    ...
    self_learning_group.add_argument('--log', default=r'./log/log1.txt', help='write to a log file in tsv format at each iteration')
    ...
    # Check command line arguments 检查在解白化之前是否进行了白化操作
    ...
    src_words, x, src_num, src_dim = embeddings.read(srcfile, dtype=dtype)
    trg_words, z, trg_num, trg_dim = embeddings.read(trgfile, dtype=dtype)
    ...
    if args.init_unsupervised:
        sim_size = min(x.shape[0], z.shape[0]) if args.unsupervised_vocab <= 0 else min(x.shape[0], z.shape[0], args.unsupervised_vocab)#保持X矩阵和Z矩阵行数相等
