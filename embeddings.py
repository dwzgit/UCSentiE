...
def read(file, threshold=0, vocabulary=None, dtype='float32'):
    ...
    stopwords = []
    matrix = []
    with open(r'./stopWord/stopWord.txt', 'r', encoding='utf-8', errors='ignore') as stopwords_f:
    ...
    return (words, matrix, len(words), dim) if vocabulary is None else (words, np.array(matrix, dtype=dtype))
