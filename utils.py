...
def get_best_C(dataset, cross_dataset):
    ...
    for i, c in enumerate(test_cs):
        ...
        pred = clf.predict(cross_dataset._Xtrain)
        if len(labels) == 2:
            dev_f1 = macro_f1(cross_dataset._ytrain, pred)
        else:
            dev_f1 = f1_score(cross_dataset._ytrain, pred, labels=labels, average='macro')
        if dev_f1 > best_f1:
            best_f1 = dev_f1
            best_c = c
...
def get_syn_ant(lang, vecs):
    ...
    synonyms1 = [l.strip() for l in open(os.path.join('syn-ant', lang, 'syn1.txt')) if l.strip() in vecs]
    synonyms2 = [l.strip() for l in open(os.path.join('syn-ant', lang, 'syn2.txt')) if l.strip() in vecs]
    neg = [l.strip() for l in open(os.path.join('syn-ant', lang, 'neg.txt')) if l.strip() in vecs]
...
def print_prediction(model, cross_dataset, outfile):
    prediction = model.predict(cross_dataset._Xtrain)
